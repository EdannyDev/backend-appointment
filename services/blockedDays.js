...
export const blockDay = async (date, reason) => {
  ...
  try {
    ...
    if (error.code === 'ER_DUP_ENTRY') {
      throw {
        status: 400,
        message: `El día ${date} ya se encuentra bloqueado`,
      };
    }
    throw error;
  }
...
export const blockRange = async (start_date, end_date, reason) => {
  if (!start_date || !end_date) {
    throw {
      status: 400,
      message: 'Fecha inicial y final son obligatorias',
    };
  }
  ...
  await db.query(
    `INSERT IGNORE INTO blocked_days (date, reason)
     VALUES ?`,
    [values]
  );
  ...
  return {
    success: true,
    message: 'Rango de fechas bloqueado correctamente',
  };
};
...
// Eliminar rango de días bloqueados
export const deleteBlockedRange = async (start_date,end_date)=>{
  ...
  const [result]=await db.query(`DELETE FROM blocked_days WHERE date BETWEEN ? AND ?`, [start_date,end_date]);
  
  if(result.affectedRows===0){
    throw{
      status:404,
      message:'No se encontraron días bloqueados en ese rango'
    };
  }
